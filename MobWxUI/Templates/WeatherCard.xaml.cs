...
public partial class WeatherCard : ContentView
{
	...
	public static readonly BindableProperty WxImageByteArrayProperty =
		BindableProperty.Create(nameof(WxImageBtyeArray), typeof(byte[]), typeof(WeatherCard), default(byte[]), BindingMode.OneWay);
	public byte[] WxImageBtyeArray
	{
		get => (byte[])GetValue(WxImageByteArrayProperty);
		set => SetValue(WxImageByteArrayProperty, value);
	}
